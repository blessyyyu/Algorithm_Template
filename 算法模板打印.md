...
void m_partition(int a[], int l, int r);
void quick_sort(int a[], int l, int r)
{
    ...
    while (!stk.empty())
    {
        ...
        stk.pop();
        m_partition(a, pr.first, pr.second);
    }
}

void m_partition(int a[], int l, int r)
{
    int p = l - 1, q = r + 1, v = a[l + r >> 1];
    while (p < q)
    {
        do p++;  while (a[p] < v);
        do q--;  while (a[q] > v);
    ...
    if (q - l + 1 > r - q)  { // 如果[l, q]区间比[q+1, r]长，则区间[l, q]先入栈，以保证栈的深度不超过logN
        if (l < q)  stk.push({l, q}); // 避免像[1，1]这种区间入栈
        if (q + 1 < r)  stk.push({q + 1, r});
    }
    else  {
        if (q + 1 < r)  stk.push({q + 1, r});
        if (l < q)  stk.push({l, q});
