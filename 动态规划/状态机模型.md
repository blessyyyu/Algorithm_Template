...
> 这个题目还有另外一种更快速的解法，使用贪心法。
>
> 由于不限制交易次数，且当天不限制买卖、知道未来的价格，所以其实可以贪心的获取所有可能的区间正收益； 
>
> 如果第二天继续涨，则我今天买入 or 继续持有； 如果第二天跌，则我当天卖出，获取现有的收益；
...
2. `f[i][1]`: 表示售出股票的当天和第一天（获取利润后+冷冻期）。
...
> https://leetcode.cn/problems/best-time-to-buy-and-sell-stock-v/submissions/740841309/  买卖股票的最佳时机五
...
public:
    long long maximumProfit(vector<int>& prices, int k) {
        ...
        long long f[n + 1][k + 1][3];   // [3]表示0,1,2; 0: 手上无股票, 1: 手上有股票, 只能看涨卖出; 2: 手上有借券，只能看跌买回
        memset(f, -0x3f, sizeof(f));
        ...
        for (int i = 1; i <= n; i ++) {
            for (int j = 1; j <= k; j ++) {
                f[i][j][0] = std::max(f[i - 1][j][0], std::max(f[i - 1][j][1] + prices[i], f[i - 1][j][2] - prices[i]));
                f[i][j][1] = std::max(f[i - 1][j - 1][0] - prices[i], f[i - 1][j][1]);
                f[i][j][2] = std::max(f[i - 1][j][2], f[i - 1][j - 1][0] + prices[i]);
            }
        }

        long long res = 0;
        for (int i = 0; i <= k; i ++)   res = std::max(res, f[n][i][0]);
        return res;
    }
};
```

> 分析： 状态机模型一样的，多一个状态是借券卖出； 
